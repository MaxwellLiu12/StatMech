...
class AutoregressiveModel(nn.Module, dist.Distribution):
    ...
    def sampler(self, logits, dim=-1): # simplified from F.gumbel_softmax
        ...
        print("gumbels size: ", gumbels.size())
        index = gumbels.max(dim, keepdim=True)[1]
        print("index size: ", index.size())
        debug = torch.zeros_like(logits, memory_format=torch.legacy_contiguous_format).scatter_(dim, index, 1.0)
        print("debug size: ", debug.size())
        return debug
    ...
    def _sample(self, sample_size: int, sampler = None):
        ...
        cache = [torch.zeros(self.nodes * self.features[0], sample_size)]
        ...
        for j in range(1, self.max_depth + 1): # iterate through depths 1:all
            for l, layer in enumerate(self.layers):
                if isinstance(layer, GraphConv): # for graph convolution layers
                    if l==0: # first layer should forward from previous depth
    ...
    def sample(self, sample_size=1):
        ...
        return cache[0].t().unflatten(1, (self.nodes, self.features[-1]))
    ...
    def sample_with_log_prob(self, sample_size=1):
        cache = self._sample(sample_size)
        sample = cache[0].t().unflatten(1, (self.nodes, self.features[-1]))
        logits = cache[-1].t().unflatten(1, (self.nodes, self.features[-1]))
